 ...
 * Se ejecuta cada minuto vía cron job.
 */
const fs = require('fs');
...
function postNotificacion(agenteId, resumen) {
...
async function main() {
  ...
  for (const ag of agentes) {
    ...
    try {
      ...
      if (!data.timestamp || !data.timestamp.startsWith(hoyFecha)) {
        fs.rmSync(file);
        continue;
      }
      const resumen = `[${ag.id}] ${data.resumen || data.tareas?.join(', ') || 'actividad'}`;
      const status = await postNotificacion(ag.id, resumen);
      if (status === 200) {
        console.log(`✓ Notificación creada: ${ag.id}`);
        fs.rmSync(file);
      } else {
        console.log(`✗ Error ${status} para ${ag.id} — no se borra`);
      }
    } catch (e) {
      // Archivo corrupto o ilegible — borrar

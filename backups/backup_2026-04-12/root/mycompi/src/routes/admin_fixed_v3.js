...
router.get('/metrics/business', ownerOnly, async (req, res) => {
  try {
    ...
    const [
      ...
      pool.query('SELECT estado, "createdAt" FROM "Trabajo" WHERE "createdAt" >= ', [inicioMes]),
      pool.query('SELECT monto FROM "Pago" WHERE "fechaPago" >= \ AND estado = ', [inicioMes, 'COMPLETED']),
      pool.query('SELECT count(*) as c FROM "Notificacion" WHERE "createdAt" >= ', [sieteDias]),
    ]);

    const clientes = clientesResult.rows;
    const agentes = agentesResult.rows;
    const trabajos = trabajosResult.rows;
    const pagos = pagosResult.rows;
    const notificaciones = notifResult.rows;

    const precios = { BASICO: 49, ESTANDAR: 99, PREMIUM: 199, COMPLETO: 49 };
    const mrrFromPagos = pagos.reduce((sum, p) => sum + (parseFloat(p.monto) || 0), 0);
    const mrrFromPlanes = clientes.reduce((sum, c) => sum + (precios[c.plan] || 49), 0);
    ...
    res.json({
      ...
      funnel,
      notificaciones7d: parseInt(notificaciones[0]?.c || 0, 10),
    });
  } catch (err) {
    console.error('[admin/metrics/business] Error:', err.message);

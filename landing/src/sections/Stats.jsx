export default function Stats() {
  const stats = [
    ...
    { num: '<24h', label: 'Primer agente operativo', sub: 'Sin implementación' },
    ...
    <section className="bg-indigo-600 py-12 md:py-16">
      <div className="max-w-[1200px] mx-auto px-6 md:px-10">
        <div className="text-center mb-8">
          <p className="text-indigo-200 text-xs font-bold uppercase tracking-widest mb-2">Por qué MyCompi</p>
          <h2 className="text-white text-xl md:text-2xl font-extrabold">Resultados que impactan tu negocio</h2>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-8">
          {stats.map(s => (
            <div key={s.label} className="text-center">
              <div className="text-4xl md:text-[52px] font-extrabold tracking-tight text-white leading-none">{s.num}</div>
              <div className="text-indigo-100 font-bold text-sm mt-2">{s.label}</div>
